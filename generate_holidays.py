...
OUTPUT_FILENAME = 'italy_holidays.csv'
HEADER = ['Year', 'Month', 'Day', 'Name']


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description = 'Years to compute holidays')
    parser.add_argument('--start_year', default = datetime.datetime.now().year, type = int, help = 'First year to be considered')
    ...
    for y in range(start_year, end_year + 1, 1):
        h = holidays.Italy(years = y)
        h[datetime.date(y, 12, 7)] = "Sant'Ambrogio Patrono di Milano" # add this local holiday for who works in Milano
    ...
    with open(OUTPUT_FILENAME, 'w') as output_file:
        ...
        for data in vacanze.keys():
            output_file.write(str(data.year) + ',' + str(data.month) + ',' + str(data.day) + ',' + vacanze[data] + '\n')
